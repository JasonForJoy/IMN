...
class IMN(object):
    def __init__(
      self, max_utter_len, max_utter_num, max_response_len, num_layer, vocab_size, embedding_size, vocab, rnn_size, maxWordLength, charVocab, l2_reg_lambda=0.0):
        ...
        self.r_charVec = tf.placeholder(tf.int32, [None, max_response_len, maxWordLength], name="response_char")
        self.r_charLen =  tf.placeholder(tf.int32, [None, max_response_len], name="response_char_len")
        ...
        with tf.name_scope('char_embedding'):
            ...
            response_char_embedded   = tf.nn.embedding_lookup(char_W, self.r_charVec)  # [batch_size, max_response_len, maxWordLength, char_dim]
        ...
        utterances_char_len      = tf.reshape(self.u_charLen, [-1])                                     # [batch_size*max_utter_num*max_utter_len, ]
        response_char_embedded = tf.reshape(response_char_embedded, [-1, maxWordLength, char_dim])      # [batch_size*max_response_len, maxWordLength, char_dim]
        response_char_len      = tf.reshape(self.r_charLen, [-1])                                       # [batch_size*max_response_len, ]
        ...
        response_cnn_char_emb = cnn_layer(response_char_embedded, filter_sizes=[3, 4, 5], num_filters=50, scope="CNN_char_emb", scope_reuse=True)      # [batch_size*max_response_len,  2*charRNN_size]
        response_cnn_char_emb = tf.reshape(response_cnn_char_emb, [-1, max_response_len, cnn_char_dim])                                                # [batch_size, max_response_len, 2*charRNN_size]
        ...
        response_embedded  = tf.concat(axis=-1, values=[response_embedded, response_cnn_char_emb])        # [batch_size, max_response_len, emb]
        ...
        with tf.variable_scope("encoding_layer") as vs:
            ...
            # r_rnn_output, r_rnn_states = lstm_layer(response_embedded, self.response_len, rnn_size, self.dropout_keep_prob, rnn_scope_name, scope_reuse=True)   # [batch_size, max_response_len, rnn_size(200)]
            # response_output = tf.concat(axis=2, values=r_rnn_output)     # [batch_size, max_response_len, 2*rnn_size(400)]
            # 2. multi_lstm_layer
